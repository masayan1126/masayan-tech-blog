 ...
 * 【用途】下書きプレビュー専用
 *
 * このコンポーネントは、下書きプレビュー時にDOM内の`span.link-card`要素を
 * Reactベースのリンクカードコンポーネントに置き換えるために使用されます。
 *
 * 本番環境では、microCMS側でリンクカードが展開されるため、このコンポーネントは使用されません。
 *
 * 動作:
 * 1. DOM内の`span.link-card`要素を検索
 * 2. 内部のアンカータグからURLを取得
 * 3. LinkCardコンポーネントでOGP情報を取得・表示
 *
 * 使用箇所:
 * - src/pages/blog/draft/index.astro（下書きプレビューページ）
 */
